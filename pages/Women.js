...
const Women = ({ women }) => {
  ...
  const [visibleProducts, setVisibleProducts] = useState(women.slice(0, 10));
      ...
      <div>
        ...
        >
          <div className="grid grid-cols-2 lg:grid-cols-3 md:grid-cols-2 mx-auto pb-10 gap-5 pt-10">
            {visibleProducts?.map((women, index) => (
              <Link key={index} href="">
                <div className="transform overflow-hidden bg-white duration-200  cursor-pointer">
                  <Image src={women.img} width={500} height={300} />
                  <div className="pt-5">
                    {/* <h6 className=" font-bold text-[#9e3500]">Just In</h6> */}
                    <h5>{women.title}</h5>
                    <h5 className="text-[#858585]">{women.category}</h5>
                    <h5 className="text-[#858585]">{women.color}</h5>
                    <div className="pt-2">
                      {women.price
                        ? women.price.toLocaleString("vi-VN", {
                            style: "currency",
                            currency: "VND",
                          })
                        : ""}
...
export async function getStaticProps() {
  const filePath = path.join(process.cwd(), "data", "women.json");
